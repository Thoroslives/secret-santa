import {
  generatePersonalLinkToken,
  generateGroupInviteCode,
  validateWishlistItems,
  isValidEmail,
} from '@/lib/utils';

const ALLOWED_CHARS = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
const EXCLUDED_CHARS = ['I', 'O', '0', '1'];

describe('generatePersonalLinkToken', () => {
  it('returns a base64url string with no padding or unsafe characters', () => {
    const token = generatePersonalLinkToken();
    expect(token).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('encodes 32 random bytes (43 base64url characters, no padding)', () => {
    const token = generatePersonalLinkToken();
    expect(token).toHaveLength(43);
  });

  it('multiple calls generate different tokens', () => {
    const tokens = new Set<string>();
    for (let i = 0; i < 20; i++) {
      tokens.add(generatePersonalLinkToken());
    }
    expect(tokens.size).toBe(20);
  });
});

describe('generateGroupInviteCode', () => {
  it('returns a 6-character string', () => {
    const code = generateGroupInviteCode();
    expect(code).toHaveLength(6);
  });

  it('only contains allowed characters', () => {
    for (let i = 0; i < 50; i++) {
      const code = generateGroupInviteCode();
      for (const char of code) {
        expect(ALLOWED_CHARS).toContain(char);
      }
    }
  });

  it('does not contain excluded characters (I, O, 0, 1, L)', () => {
    for (let i = 0; i < 50; i++) {
      const code = generateGroupInviteCode();
      for (const excluded of EXCLUDED_CHARS) {
        expect(code).not.toContain(excluded);
      }
    }
  });

  it('multiple calls usually generate different codes', () => {
    const codes = new Set<string>();
    for (let i = 0; i < 20; i++) {
      codes.add(generateGroupInviteCode());
    }
    expect(codes.size).toBeGreaterThan(1);
  });
});

describe('validateWishlistItems', () => {
  const validItem = { title: 'Cool Gadget', note: 'Any color is fine' };

  describe('valid cases', () => {
    it('returns valid for 1 valid item', () => {
      const result = validateWishlistItems([validItem]);
      expect(result).toEqual({ valid: true });
    });

    it('returns valid for 5 valid items', () => {
      const items = Array.from({ length: 5 }, (_, i) => ({
        title: `Item ${i}`,
        note: `Note ${i}`,
      }));
      const result = validateWishlistItems(items);
      expect(result).toEqual({ valid: true });
    });

    it('returns valid when note is omitted entirely', () => {
      const result = validateWishlistItems([{ title: 'Cool Gadget' }]);
      expect(result).toEqual({ valid: true });
    });

    it('returns valid when note is an empty string', () => {
      const result = validateWishlistItems([{ title: 'Cool Gadget', note: '' }]);
      expect(result).toEqual({ valid: true });
    });

    it('returns valid when note is a plain-text description', () => {
      const result = validateWishlistItems([{ title: 'Cool Gadget', note: 'Size medium, any color' }]);
      expect(result).toEqual({ valid: true });
    });

    it('returns valid when note is a URL', () => {
      const result = validateWishlistItems([{ title: 'Cool Gadget', note: 'https://example.com/gadget' }]);
      expect(result).toEqual({ valid: true });
    });
  });

  describe('empty list', () => {
    it('returns valid for an empty array (clears the wishlist)', () => {
      const result = validateWishlistItems([]);
      expect(result).toEqual({ valid: true });
    });
  });

  describe('invalid item counts', () => {
    it('returns error for 6 items', () => {
      const items = Array.from({ length: 6 }, (_, i) => ({
        title: `Item ${i}`,
        note: `Note ${i}`,
      }));
      const result = validateWishlistItems(items);
      expect(result.valid).toBe(false);
      expect(result.error).toMatch(/maximum.*5/i);
    });

    it('returns error for more than 6 items', () => {
      const items = Array.from({ length: 10 }, (_, i) => ({
        title: `Item ${i}`,
        note: `Note ${i}`,
      }));
      const result = validateWishlistItems(items);
      expect(result.valid).toBe(false);
      expect(result.error).toMatch(/maximum.*5/i);
    });
  });

  describe('invalid titles', () => {
    it('returns error for empty title', () => {
      const result = validateWishlistItems([{ title: '', note: 'x' }]);
      expect(result.valid).toBe(false);
      expect(result.error).toMatch(/title/i);
    });

    it('returns error for whitespace-only title', () => {
      const result = validateWishlistItems([{ title: '   ', note: 'x' }]);
      expect(result.valid).toBe(false);
      expect(result.error).toMatch(/title/i);
    });

    it('returns error for empty title even when note is omitted', () => {
      const result = validateWishlistItems([{ title: '' }]);
      expect(result.valid).toBe(false);
      expect(result.error).toMatch(/title/i);
    });
  });

  describe('mixed valid and invalid items', () => {
    it('returns error if any item has empty title', () => {
      const items = [
        { title: 'Good', note: 'fine' },
        { title: '', note: 'also fine but no title' },
      ];
      const result = validateWishlistItems(items);
      expect(result.valid).toBe(false);
      expect(result.error).toMatch(/title/i);
    });

    it('returns error if the first item is invalid even when later items are fine', () => {
      const items = [
        { title: '   ', note: 'blank title' },
        { title: 'Good', note: 'fine' },
      ];
      const result = validateWishlistItems(items);
      expect(result.valid).toBe(false);
      expect(result.error).toMatch(/title/i);
    });
  });
});

describe('isValidEmail', () => {
  it.each([
    'a@b.co',
    'alice@example.com',
    'first.last+tag@sub.domain.com',
    "o'brien@example.co.uk",
  ])('accepts %s', (email) => {
    expect(isValidEmail(email)).toBe(true);
  });

  it.each([
    ['', 'empty'],
    ['a', 'no @ and no domain'],
    ['a@', 'no domain'],
    ['@b.com', 'no local part'],
    ['a@b', 'no dot in the domain'],
    ['a b@c.com', 'whitespace in the local part'],
    ['a@b .com', 'whitespace in the domain'],
    ['a@@b.com', 'two @ signs'],
    ['a@b.', 'trailing dot, no TLD'],
  ])('rejects %s (%s)', (email) => {
    expect(isValidEmail(email)).toBe(false);
  });
});
